...
function scene:create( event )
    ...
    local categories = MAIN.UI.CONTAINER.createScrollViewWithRectangles({
        ...
        touch=function(input)
            ...
            --it messes up when coming from floor, im not bother fixing it
            --sceneGroup[2]:scrollToCurrent()
        end
    })
    ...
    function categories:scrollToCurrent()
        ...
        sceneGroup[1]:selectAndCall(part)
        sceneGroup[2]:scrollToIndex(stage)
    end
    ...
    local stages =  MAIN.UI.COMMON.createStagesScrollView({
        ...
        touch=function(input)
            ...
            local stage = (categories.currentIndex - 1) * 6 + input.index
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        if stage then
            ...
            if stage > 6 then
                sceneGroup[1]:selectAndCall(2, true)
                stage = stage - 6
                if not sceneGroup[2]:isIndexVisible(stage) then
                    sceneGroup[2]:scrollToIndex(stage)
                else
